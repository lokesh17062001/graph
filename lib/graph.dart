...
class Graphcls extends CustomPainter{
  ...
  void paint(Canvas canvas, Size size)
  {
    ...
    {
      x.add((double.parse(myData['x'][i].substring(11,13)))*3600 +(double.parse(myData['x'][i].substring(14,16)))*60+(double.parse(myData['x'][i].substring(17,19))));
    }
    ...
    for(int i=0;i<x.length;i++)
    {
      x[i]=(x[i] - x11[0]) * (size.width-50-50) / (x11[x11.length-1] - x11[0]) + 50;
    }
    ...
    for(int i=0;i<x.length;i++)
    {
      points.add(Offset((x[i]),size.height-50-myData['y'][i]*(size.height+50-l1[l1.length-1])/l1[l1.length-1]));
    }
    // print((size.height-50-l1[l1.length-1])/l1[l1.length-1]);
    ...
    for(int i=0;i<l.length-1;i++)
    {
      ...
      Offset point2=Offset(size.width-50-1.5,size.height-50-(l[i]+l[i+1])/2);
    ...
    for(int i=0;i<l.length-1;i++)
    {
      ...
      tp2.layout();
      Offset point4=Offset(size.width-50+5,size.height-50-(l[i+1]));
    ...
    var count=0;
    for(var i=25;i<n+50&& count<=numOfPoints+1;i+=50,count+=1)
    {
      x1.add(i);
    ...
    for(int i=0;i<a.length;i++)
    {
      ...
      TextSpan span = TextSpan(style:const TextStyle(color: Colors.black), text:b[i]);
      TextPainter tp = TextPainter(text: span, textDirection: TextDirection.ltr);
      tp.layout();
      Offset point=pl[i];
      tp.paint(canvas,point);
    }//x-labels
    var text=[myData['y1label'],myData['y2label'],myData['xlabel']+'--->',myData['xlabel']+'--->'];
    var plots=[Offset(50/2-5,size.height-80-(l[l.length-1])), Offset(size.width-45,size.height-80-(l[l.length-1])),Offset(size.width-50-text[3].length*8,size.height-25),Offset(50,size.height-25)];

    for(int i=0;i<text.length;i++)
    {
      TextSpan span = TextSpan(style:const TextStyle(color: Colors.black,fontWeight: FontWeight.bold), text:text[i]);
      TextPainter tp = TextPainter(text: span, textDirection: TextDirection.ltr);
      tp.layout();
      Offset point=plots[i];
      tp.paint(canvas,point);
    }
    TextSpan span = TextSpan(style:const TextStyle(color: Colors.black,fontWeight: FontWeight.bold,fontSize: 30), text:myData['title']);
    TextPainter tp = TextPainter(text: span, textDirection: TextDirection.ltr);
    tp.layout();
